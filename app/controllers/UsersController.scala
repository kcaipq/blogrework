...
class UsersController(actions: ControllerActions[Users, UserEvent], claimEmailAddress: (EmailAddress, UserId) => UserId) {
  ...
  private val loginForm = Form(tuple("email" -> support.Forms.emailAddress, "password" -> password))
  ...
  private val registrationForm: Form[(EmailAddress, String, Password)] = Form(tuple(
    "email" -> support.Forms.emailAddress,
  ...
  private def changeEmailForm(implicit user: RegisteredUser) =
    Form(single("email" -> support.Forms.emailAddress.verifying("value.unchanged", _ != user.email))
